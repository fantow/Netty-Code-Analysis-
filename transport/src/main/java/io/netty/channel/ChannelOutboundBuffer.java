...
public final class ChannelOutboundBuffer {
    ...
    public void addMessage(Object msg, int size, ChannelPromise promise) {
        ...
        // 调整entry的位置
        // 这里可以去看一下Entry的定义，其实这个链表的结构是这样的：
        // Entry(flushedEntry) --> XXX --> Entry(unflushedEntry) --> XXX --> Entry(tailEntry)
        // 一开始加入entry时，flushedEntry指向null，unflushedEntry和tailEntry都指向新加入的Entry节点
        if (tailEntry == null) {
            flushedEntry = null;
        } else {
    ...
    public void addFlush() {
        ...
        if (entry != null) {
            // 如果flushedEntry为null，说明目前还没有进行过flush操作
            if (flushedEntry == null) {
                // there is no flushedEntry yet, so start with the entry
                // 将flushEntry指向第一个entry
                flushedEntry = entry;
            }
            do {
                flushed ++;
                if (!entry.promise.setUncancellable()) {
                    ...
                    // 这里会进行高水位的减操作(因为有数据从buffer中flush掉了，并且当buffer的mark低于阈值(32k)时
                    // 将buffer设置为可写状态)
                    // 也就是说，当buffer超过64k时，将buffer设置为不可写状态
                    // 当buffer小于32k时，将buffer设置为可写状态
    ...
    private void incrementPendingOutboundBytes(long size, boolean invokeLater) {
        ...
        // 这里有提到一个highWaterMark，默认最大值为64k，即如果超过buffer的最大容量，会直接返回
        // 不可写
    ...
    public boolean remove() {
        ...
        // 移除这个entry，移除逻辑就是将flushedEntry指针指向这个entry即可
        removeEntry(e);
