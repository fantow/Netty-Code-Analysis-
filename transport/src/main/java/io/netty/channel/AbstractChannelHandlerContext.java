...
// implements的ChannelHandlerContext继承了Attribute(支持存储配置信息),
//      ChannelInboundInvoker:用来支持对入栈事件(read，register，active)的传播
//      ChannelOutboundInvoker:用来支持对出栈事件的传播
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    // 如果handler中需要向后传递exception，就需要调用ctx.fireException()
    // 如果需要在某个handler中对exception进行处理，需要实现exceptionCaught()方法
    @Override
    public ChannelHandlerContext fireExceptionCaught(final Throwable cause) {
        // 通过调用findContextInbound寻找下一个ctx，这里不要求下一个是inbound还是outbound
    ...
    private void invokeExceptionCaught(final Throwable cause) {
        if (invokeHandler()) {
            try {
                // 调用handler中对应的exceptionCaught()方法
    ...
    private void invokeRead() {
        if (invokeHandler()) {
            try {
                // 这里其实调的是DefaultChannelPipeline的 unsafe.beginRead();
    ...
    private void invokeWrite0(Object msg, ChannelPromise promise) {
        try {
            // 这里会去调下一个Handler的write方法(可以用MessageToByteEncoder这个类的write去看)，
            // 如果没有实现，就执行其父类的write方法
    ...
    void invokeWriteAndFlush(Object msg, ChannelPromise promise) {
        if (invokeHandler()) {
            // 这个方法会去调用ChannelOutboundHandler的write方法
            // 具体逻辑可以去找个MessageToByteEncoder的write去看
            invokeWrite0(msg, promise);
            // 这个也是去调用ChannelOutboundHandler的flush方法
            // 一般这个flush方法，其实就是调用ctx.flush()，将flush事件向head方法进行传播
            invokeFlush0();
        } else {
    ...
    // 在一般OutboundHandler的write中，一般都要调用ctx.write，来实现写入操作
    private void write(Object msg, boolean flush, ChannelPromise promise) {
        ...
        // 找到Pipeline中，前一个outBoundHandler
        ...
        // 判断是否是eventLoop中的线程
        // 如果是，则直接调用flush
        // 如果不是，则将写事件封装为一个任务，放入queue中等待执行
        if (executor.inEventLoop()) {
            // 调用invokeWrite()方法，将数据写入
            if (flush) {
                // 这里面会分别调用invokeWrite0和invokeFLush0方法
                next.invokeWriteAndFlush(m, promise);
            } else {
                // 在这个invokeWrite()中，会调用这个context对应的handler的write()方法
    ...
    final void callHandlerAdded() throws Exception {
        ...
        // 是通过cas自旋，将 oldState状态转为ADD_COMPLETE
        if (setAddComplete()) {
            // 这里会去调用handler中的handlerAdded方法
            // 对于比较特殊的handler，比如ChannelInitializer，会在其handlerAdded中，去执行initChannel方法
