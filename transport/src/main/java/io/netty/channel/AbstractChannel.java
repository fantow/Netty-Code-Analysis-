...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    // 这4个属性，在Accept后，会通过new NioSocketChannel()时创建
    ...
    public ChannelFuture writeAndFlush(Object msg) {
        // 所有通过ctx.channel().writeAndFlush()的方法，都会通过调用pipeline.writeAndFlush()实现
        // 并且在pipeline中，也会去调用tail.writeAndFlush，就是说，通过channel调用输出时，都是从tail开始
        // 向head传播的
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            // 这里传入的eventLoop，其实就是执行bootstrap.bind()这个指定的那个线程
            AbstractChannel.this.eventLoop = eventLoop;

            // 一般到这里的线程，都是BossGroup的EventLoop，所以一定不在WorkerGroup中
            if (eventLoop.inEventLoop()) {
                register0(promise);
            } else {
                try {
                    // 这里会调用WorkerGroup中的EventLoop执行register0()方法
        ...
        // 这个register0其实执行了：将eventLoop注册给Channel，以及为Channel注册读事件
        private void register0(ChannelPromise promise) {
            try {
                ...
                // 触发add回调函数
                // 这个是用户在配置bootstrap的Handler中可能带有的handlerAdded()方法
                pipeline.invokeHandlerAddedIfNeeded();

                safeSetSuccess(promise);

                // 注册成功后的通知
                // 如果用户注册的Handler中有channelRegistered()方法，就会在这里被调用
                pipeline.fireChannelRegistered();
                // Only fire a channelActive if the channel has never been registered. This prevents firing
                // multiple channel actives if the channel is deregistered and re-registered.

                // 到这里，新连接已经接入，并且已经被注册eventLoop了
                if (isActive()) {
                    // 如果是第一次注册成功，就要执行channelActive()方法
        ...
        public final void write(Object msg, ChannelPromise promise) {
            ...
            int size;
            try {
                // 一般去追AbstractNioByteChannel类的这个方法
            ...
            // 写buffer操作，主要作用是：将msg封装为entry并转换为堆外内存，将其插入写缓冲区，设置缓冲区状态
        ...
        protected void flush0() {
            ...
            try {
                // 对buffer队列进行遍历，并且过滤ByteBuf，最终写入JDK底层socket中
                // 进入到AbstractNioByteChannel
